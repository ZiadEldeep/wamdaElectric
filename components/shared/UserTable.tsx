  ...
  return (
    <Container component={motion.div} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      <Drawer anchor="bottom" open={drawerOpen} onClose={handleCloseDrawer}>
        ...
        <AddUserPage user={selectedUser} onClose={handleCloseDrawer} refetch={refetch} />
        </div>
      </Drawer>
    </Container>
  );
...
const AddUserPage: React.FC<{ user: any; onClose: () => void;refetch:()=>void }> = ({ user, onClose,refetch }) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
    ...
    defaultValues: user || { name: '', email: '', password: '', role: '' }, // Set default values
  });

  const onSubmit = async (data: UserFormData) => {
    try {
      // Modify this part to call an update API instead
      const result = await axios.put(`/api/users/${user._id}`, data); // Change this to updateUser if needed
